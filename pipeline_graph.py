...
def generate_learning_event(state: PipelineState) -> PipelineState:
    ...
    llm_result, llm_status = _llm_json(
        system_prompt=(
            ...
            "Your output must be story-driven, exam-focused, and easy to read on screen. "
        ...
        user_prompt=(
            ...
            "4) story: 250–400 words per story_card. Use SHORT paragraphs for readability.\n"
            "5) Format the story with CLEAR structure: separate paragraphs with a blank line (double newline). "
            "   Use 3–5 paragraphs. The INTRO (first paragraph) must be LONGER: 4–6 sentences that set context, "
            "   introduce the topic, and hook the reader. Middle paragraphs = 2–4 sentences each (explanation + example). "
            "   Last paragraph = takeaway or exam tip (2–3 sentences).\n"
            "6) Do NOT format any formal quiz (no MCQ, no answer key blocks).\n"
            "7) friend_explainers: exactly 2–4 short, punchy lines (one sentence each). "
            "   Like a friend summarizing out loud: conversational, memorable.\n"
            "8) Keep output practical for exam preparation; avoid fluff and walls of text.\n"
            "9) checklist must be topic/subtopic-driven and exam actionable.\n\n"
            "10) Write in second-person: the learner is always 'you'.\n"
            "11) Each story should flow: setup -> explanation (with example) -> takeaway.\n"
            "12) Use simple, scannable sentences. Avoid long nested clauses.\n"
            "13) CRITICAL: NEVER repeat the same paragraph or the same sentence. Every paragraph must "
            "    be unique in content. Do NOT use the same filler phrase (e.g. 'You pause at your desk...') "
            "    in multiple paragraphs. Write 3–5 distinct paragraphs only.\n\n"
...
def generate_story_visuals(state: PipelineState) -> PipelineState:
    ...
    result, _ = _llm_json(
        system_prompt=(
            ...
            "     Step 1: overview diagram (framework, definition, or key idea) — shown early in the topic.\n"
            "     Step 2: detail diagram (process, comparison, or worked example) — shown after the main explanation.\n"
